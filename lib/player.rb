...
class Player
  ...
  def valid_random_coordinate_array_cruiser
    coords = random_coordinate_array(@ships[0].length)
    until @board.valid_placement?(@ships[0], coords)
      coords = random_coordinate_array(@ships[0].length)
    end
    coords
  end
  ...
  def valid_random_coordinate_array_submarine
    coords = random_coordinate_array(@ships[1].length)
    until @board.valid_placement?(@ships[1], coords)
      coords = random_coordinate_array(@ships[1].length)
    end
    coords
  end
  ...
  def let_human_place_cruiser
    ...
    until @board.valid_placement?(@ships[0], desired_placement)
  ...
  def let_human_place_submarine
    ...
    until @board.valid_placement?(@ships[1], desired_placement)
      puts 'Those are invalid coordinates. Please try again:'
      desired_placement = gets.chomp.upcase.split
    end
